...
- Mobile Supabase config previously fell back to a local development URL/key, which could hide missing production credentials.
...
- Missing `EXPO_PUBLIC_SUPABASE_URL` or `EXPO_PUBLIC_SUPABASE_ANON_KEY` now shows a blocking configuration error instead of silently using local development credentials.
...
### App Store blockers
- EAS/TestFlight must still be run with production `EXPO_PUBLIC_SUPABASE_URL` and `EXPO_PUBLIC_SUPABASE_ANON_KEY` values.

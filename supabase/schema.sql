...
select
  ...
  pr.following_count,
  pr.posts_count,
  pr.username_last_changed_at,
  exists (select 1 from public.likes l where l.post_id = p.id and l.user_id = auth.uid()) as liked_by_me,
  exists (select 1 from public.reposts r where r.post_id = p.id and r.user_id = auth.uid()) as reposted_by_me,
...
create or replace function public.enforce_username_change_window() returns trigger language plpgsql as $$
begin
...
drop trigger if exists profiles_enforce_username_change_window on public.profiles;
create trigger profiles_enforce_username_change_window before update on public.profiles for each row execute function public.enforce_username_change_window();

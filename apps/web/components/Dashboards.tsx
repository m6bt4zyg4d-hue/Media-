...
function ActionButton({ children }: { children: React.ReactNode }) {
  ...
  return <span><button className="ghost" onClick={() => setMessage('Action queued.')}>{children}</button>{message && <span className="muted"> {message}</span>}</span>;
}
...
export function ModerationDashboard() {
  return <div className="stack" style={{ padding: 20 }}>{demoModerationQueue.map((item) => <article className="card" key={item.id} style={{ border: '1px solid var(--border)', borderRadius: 20 }}><div className="row space"><strong>{item.targetType} · {item.targetId}</strong><span className="tab">{item.status}</span></div><p>{item.aiReason}</p><p className="muted">AI score: {item.aiScore}</p><div className="tabs"><ActionButton>Approve</ActionButton><ActionButton>Remove</ActionButton><ActionButton>Escalate</ActionButton><ActionButton>Open appeal</ActionButton></div></article>)}</div>;
}
